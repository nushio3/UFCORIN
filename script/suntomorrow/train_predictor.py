...
model=chainer.FunctionSet(
    conv1 = F.Convolution2D(1,4,3,stride=1,pad=1),
    conv9 = F.Convolution2D(8,1,3,stride=1,pad=1),
)
...
def forward(x_data,train=True):
    ...
    h1 = F.dropout(F.relu(model.conv1(x)),  train=train)
    h2 = F.average_pooling_2d(h1,2)
    h3 = F.concat([h2,h2], 2)
    h4 = F.concat([h3,h3], 3)
    h5 = F.concat([h1,h4])
    h6 = model.conv9(h5)
    return F.mean_squared_error(x,h6)
